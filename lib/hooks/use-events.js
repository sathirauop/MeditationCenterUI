 ...
 * Supports both text field updates and image uploads
 */
export function useUpdateEvent() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: ({ id, eventData, coverImage, galleryImages }) =>
      updateEvent(id, eventData, coverImage, galleryImages),
 ...
 * @returns {Object} Mutation object with { event_id, message, images_deleted }
 */
export function useDeleteEvent() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (eventId) => api.delete(`/admin/event/${eventId}`),
    onSuccess: () => {
