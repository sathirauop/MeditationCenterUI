...
/**
 * Update an existing program (multipart - supports text fields + images)
 * @param {number} programId - Program ID
 * @param {Object} data - Partial program data to update
 * @param {File} [coverImage] - Optional new cover image file
 * @param {File[]} [galleryImages] - Optional new gallery image files
 * @returns {Promise<Object>} Updated program
 */
export const updateProgram = async (programId, data, coverImage, galleryImages) => {
    const formData = new FormData();
    ...
    return await axiosClient.patch(`${API_BASE}/programs/${programId}`, formData, {
        headers: {
            'Content-Type': 'multipart/form-data',
        },
    });
