import axiosClient, { refreshAuthToken } from '../axios-client';
      ...
      if (token) {
        ...
        if (isTokenExpired(token)) {
          ...
          try {
            ...
            // Refresh succeeded — decode the fresh token
            ...
            // Tokens are already cleared inside refreshAuthToken()
            throw new Error('Session expired. Please login again.');
          }
        }

        // Token is still valid — decode and return
        try {
 ...
 * Uses a 30-second buffer so we refresh slightly before
 * actual expiry, avoiding edge-case race conditions.
 *
 * @param {string} token - The JWT string
 * @returns {boolean} true if expired (or un-decodable)
 */
