...
export const getActiveTemplate = async () => {
    return await axiosClient.get(`${API_BASE}/templates/active`);
};
...
export const deleteOverrideByDate = async (date) => {
    return await axiosClient.delete(`${API_BASE}/overrides/by-date/${date}`);
};
...
export const getSchedulePreview = async (date) => {
    return await axiosClient.get(`${PUBLIC_API_BASE}/${date}`, { params: { date } });
};

export const getTodaySchedule = async () => {
    return await axiosClient.get(`${PUBLIC_API_BASE}/today`);
};
